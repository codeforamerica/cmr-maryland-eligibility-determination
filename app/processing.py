...
def determine_eligibility(state, df):
    if state == "Maryland":
        print("Running eligibility for Maryland")
        return determine_maryland_eligibility(df)
    else:
        print("Running eligibility for Hawaii")
        return determine_hawaii_eligibility(df)

# TODO: modify this to handle Hawaii-specific rules
def determine_hawaii_eligibility(df):
    """
    ...
    for case_number, case_df in df.groupby("Case Number"):
        ...
        def calculate_eligibility_date(row):
            ...
            if row["Is Non-Conviction"]:
                return row["Disposition Date"] + timedelta(days=WAIT_PERIODS["Maryland"]["non_conviction"])
            else:
                return row["Disposition Date"] + timedelta(days=WAIT_PERIODS["Maryland"]["misdemeanor"])
...
def determine_maryland_eligibility(df):
    """
    Determines eligibility for record clearance based on charge type, disposition,
    and waiting periods. Applies exclusion rules for felonies, domestic violence,
    and specific misdemeanor statutes.
...
def process_case_data(parties_df, cases_df, charges_df, show_errors=True, state="Maryland"):
    """
    ...
    try:
        ...
        merged_df = determine_eligibility(
            state=state,
            df=categorize_charges(clean_dataframe(merged_df))
        )
